...
def detect_objects(input_bev_maps, model, configs):
    ...
    if len(detections) > 0:
        ...
        ## step 3 : perform the conversion using the limits for x, y, and z set in the configs structure
            ...
            min_x, max_x = configs.lim_x
            min_y, max_y = configs.lim_y
            min_z, max_z = configs.lim_z
        
            x = bev_y / configs.bev_height * (x_range)
            y = bev_x / configs.bev_width * (y_range)-(y_range/2.0)
            w = bev_w / configs.bev_width * (y_range)
