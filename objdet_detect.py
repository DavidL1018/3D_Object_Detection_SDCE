...
def create_model(configs):
    ...
    if (configs.arch == 'darknet') and (configs.cfgfile is not None):
        ...
        model=fpn_resnet.get_pose_net(num_layers=18, heads=configs.heads, head_conv=configs.head_conv, imagenet_pretrained=False)
        #######
        ####### ID_S3_EX1-4 END #######     
    ...
    if len(detections)>0:
       ...
       ## step 2 : loop over all detections
        for obj in detections:
            idx,bev_x,bev_y,z,h,bev_w,bev_l,yaw = obj
        
            ## step 3 : perform the conversion using the limits for x, y and z set in the configs structure
            min_x,max_x = configs.lim_x[0],configs.lim_x[1]
            min_y,max_y = configs.lim_y[0],configs.lim_y[1]
            min_z,max_z = configs.lim_z[0],configs.lim_z[1]
        
            x = bev_y / configs.bev_height * (max_x - min_x)
            y = bev_x / configs.bev_width * (max_y - min_y)-(max_y - min_y)/2
            w = bev_w / configs.bev_width * (max_y - min_y)
            l = bev_l / configs.bev_height * (max_x - min_x)
