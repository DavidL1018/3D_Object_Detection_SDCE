...
class Association:
    ...
    def associate(self, track_list, meas_list, KF):
        ...
        self.unassigned_tracks=[i for i in range(N) if np.all(self.association_matrix[i,:]==np.inf)]
        self.unassigned_meas=[j for j in range(M) if np.all(self.association_matrix[:,j]==np.inf)]
    ...
    def MHD(self, track, meas, KF):
        ...
        return gamma.transpose()*S_*gamma
